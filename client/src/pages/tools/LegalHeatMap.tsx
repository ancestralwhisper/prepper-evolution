...
/* ================================================================
   LegalHeatMap  --  OHV Legal-Status Heat Map (50 US States)
   ================================================================
   Real geographic SVG paths (Albers USA projection, 960x593 viewBox).
   Path data sourced from react-usa-map (MIT licensed).
   ================================================================ */
...
// Albers USA projection fitted to 960x593 viewBox.
...
// ─── Label positions (manually centered on each state) ────────────
...
// Small states that get smaller font
const SMALL_STATES = new Set(["CT", "DE", "MA", "MD", "NH", "NJ", "RI", "VT"]);

// ─── Legend labels by view ─────────────────────────────────────────
    ...
    <div className="bg-muted border border-border rounded-lg p-4 space-y-4">
      ...
      <div className="relative overflow-x-auto">
        ...
        >
          ...
          {Object.entries(STATE_PATHS).map(([code, d]) => {
            ...
            return (
              <path
                ...
                d={d}
                fill={fill}
                stroke={isSelected ? "#C45D2C" : "rgba(0,0,0,0.3)"}
          ...
          {/* State labels */}
          {Object.entries(LABEL_POS).map(([code, [x, y]]) => {
            const isSmall = SMALL_STATES.has(code);
            return (
              <text
                key={`label-${code}`}
                x={x}
        ...
        {/* Hover tooltip */}
        {hoveredState && (
          <div className="absolute top-2 right-2 bg-background border border-border rounded-md px-3 py-1.5 shadow-lg pointer-events-none z-10">
